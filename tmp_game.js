document.addEventListener('DOMContentLoaded', async () => {
    ...
    (function () {
        ...
        async function init() {
            ...
            // Check if admin has opened round 1 before allowing game
            try {
                ...
                if (json.ok && json.state) {
                    ...
                    if (!state.is_active || state.max_question_index < 1) {
                        // Admin chưa mở câu 1 → quay về register (đã có tên)
                        window.location.href = '/register.html';
                        return;
                    }
                    // Cập nhật adminMaxDay từ server
                    adminMaxDay = state.max_question_index;
                }
            } catch (e) {
                // Server không chạy hoặc lỗi → cho chơi bình thường (offline mode)
                console.warn('[Game] Không thể kiểm tra game_state, chạy offline mode.');
            }
...
}   // <-- this brace closes the async function
);  // <-- this closes the addEventListener call